...
def votar_restaurante(request, restaurante_id):
    ...
    try:
        ...
        return Response({"detail": "Solo los votantes pueden votar."}, status=status.HTTP_403_FORBIDDEN)

    cedula = request.data.get('cedula', '').strip()
    if not cedula:
        return Response({"detail": "La cédula es requerida para votar."}, status=status.HTTP_400_BAD_REQUEST)

    if votante.cedula and votante.cedula != cedula:
        return Response({"detail": "La cédula no coincide con la registrada."}, status=status.HTTP_400_BAD_REQUEST)

    if not votante.cedula:
        if Votante.objects.filter(cedula=cedula).exists():
            return Response({"detail": "Esta cédula ya está registrada por otro usuario."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        ...
        return Response({"detail": "Restaurante no encontrado o no disponible."}, status=status.HTTP_404_NOT_FOUND)

    if Voto.objects.filter(usuario=votante).exists():
        return Response({"detail": "Ya usaste tu voto. Solo se permite votar por un restaurante."}, status=status.HTTP_400_BAD_REQUEST)

    Voto.objects.create(usuario=votante, restaurante=restaurante)
    restaurante.votos_total += 1
    restaurante.save()

    return Response({"mensaje": f"Voto registrado para {restaurante.nombre}."}, status=status.HTTP_201_CREATED)
...
def eliminar_voto(request, restaurante_id):
    ...
    try:
        ...
        return Response({"detail": "Solo los votantes pueden realizar esta acción."}, status=status.HTTP_403_FORBIDDEN)
    ...
    try:
        ...
        return Response({"detail": "No tienes un voto registrado para este restaurante."}, status=status.HTTP_404_NOT_FOUND)
    ...
    return Response({"mensaje": f"Voto eliminado para {restaurante.nombre}."}, status=status.HTTP_200_OK)
