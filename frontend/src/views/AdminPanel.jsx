  ...
  const handleToggle = async (id) => {
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/api/restaurantes/${id}/toggle/`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      if (res.ok) {
        ...
        setRestaurantes((prev) =>
          prev.map((r) =>
            r.id === id ? { ...r, habilitado: data.habilitado } : r,
          ),
        );
      }
    } catch {
        ...
        <tbody>
          {restaurantes.map((r) => (
            <tr key={r.id}>
              ...
              <td>
                <span
                  className={`panel-badge ${r.habilitado ? "panel-badge--on" : "panel-badge--off"}`}
                >
              ...
              <td className="panel-acciones-td">
                ...
                {/* Botón Editar — navega pasando los datos actuales como state */}
                <button
                  className="panel-toggle panel-toggle--edit"
                  onClick={() =>
                    navigate(`/admin/editar-participante/${r.id}`, {
                      state: { restaurante: r },
                    })
                  }
                >
...
export default function AdminPanel() {
  ...
  useEffect(() => {
    if (location.state?.mensajeExito) {
      ...
      // limpia el state para que no reaparezca si el usuario recarga
