...
import "./CrearParticipante.css"; // Reutiliza los estilos existentes

// ── Valor inicial vacío (se sobreescribe con los datos reales) ──
const CAMPO_VACIO = {
  nombre: "",
  descripcion: "",
  direccion: "",
...
// ── Convierte el objeto restaurante del backend al shape del form ──
...
export default function EditarParticipante() {
  ...
  const { id } = useParams(); // /admin/editar-participante/:id
  const location = useLocation(); // state que mandó AdminPanel

  const [form, setForm] = useState(CAMPO_VACIO);
  const [errores, setErrores] = useState({});
  const [mensaje, setMensaje] = useState(null);
  const [cargando, setCargando] = useState(false);
  const [iniciado, setIniciado] = useState(false); // evita parpadeo del form

  useEffect(() => {
    if (!iniciado) return;
    // Busca todos los textareas y les ajusta el alto al contenido actual
    document.querySelectorAll(".cp-field textarea").forEach((el) => {
      el.style.height = "auto";
      el.style.height = el.scrollHeight + "px";
    });
  }, [iniciado]); // se ejecuta cuando los datos terminan de cargar
  ...
  // ── Precargar datos ──
  useEffect(() => {
    // Si AdminPanel pasó el restaurante via state, úsalo directamente
    ...
    // Fallback: obtener del backend si el usuario entró por URL directa
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/api/restaurantes/${id}/editar/`,
        {
          method: "PUT",
          headers: {
      ...
      if (res.ok) {
        navigate("/admin/panel", {
          state: {
            mensajeExito: `Restaurante "${data.nombre}" actualizado correctamente.`,
          },
        });
        return;
      } else {
  ...
  if (!iniciado)
    return (
      <div className="cp-wrapper">
        <p style={{ padding: 40, color: "#aaa" }}>Cargando...</p>
      </div>
    );
  ...
  return (
    <div className="cp-wrapper">
      <header className="cp-header">
        <button className="cp-back" onClick={() => navigate("/admin/panel")}>
          ← Volver
        </button>
      ...
      <form className="cp-form" onSubmit={handleSubmit} noValidate>
        ...
        <section className="cp-section">
          ...
          <div className="cp-field">
            ...
            />
            {errores.nombre && (
              <span className="cp-error">{errores.nombre}</span>
            )}
          </div>
          ...
          <div className="cp-field">
            ...
            />
            {errores.descripcion && (
              <span className="cp-error">{errores.descripcion}</span>
            )}
          </div>
          ...
          <div className="cp-field">
            ...
            <input
              ...
              onChange={handleChange}
              placeholder="Ej: Calle 35 # 10-20, Cúcuta"
              required
            />
            {errores.direccion && (
              <span className="cp-error">{errores.direccion}</span>
            )}
          </div>
          ...
          <div className="cp-row">
            ...
            <div className="cp-field">
              ...
              />
              {errores.video_url && (
                <span className="cp-error">{errores.video_url}</span>
              )}
            </div>
          </div>
        </section>
        ...
        <section className="cp-section">
          ...
          <div className="cp-field">
            ...
            <input
              ...
              required
            />
            {errores.plato_nombre && (
              <span className="cp-error">{errores.plato_nombre}</span>
            )}
          </div>
          ...
          <div className="cp-field">
            ...
            />
            {errores.plato_imagen_url && (
              <span className="cp-error">{errores.plato_imagen_url}</span>
            )}
          </div>
        ...
        <div className="cp-actions">
          ...
          <button
            type="submit"
            className="cp-btn cp-btn--primario"
            disabled={cargando}
          >
