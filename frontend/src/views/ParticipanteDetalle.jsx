...
export default function ParticipanteDetalle() {
  ...
  // Verifica si el usuario ya votó por este restaurante
  useEffect(() => {
    ...
    fetch(`http://127.0.0.1:8000/api/interacciones/${id}/verificar-voto/`, {
      ...
      .then((r) => r.json())
      .then((data) => setVotado(data.votado))
      .catch(() => {});
  }, [user, restaurante]);
  ...
  const handleVotar = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        mostrarToast(data.detail || "No se pudo registrar el voto.");
      }
    } catch {
  ...
  const handleEliminarVoto = async () => {
    try {
      ...
      if (res.ok) {
        setVotado(false);
        mostrarToast("Voto eliminado correctamente.", "info");
      } else {
        mostrarToast(data.detail || "No se pudo eliminar el voto.");
      }
    } catch {
      mostrarToast("Error de conexión. Intenta de nuevo.");
    }
  ...
  const toastUI = toast && (
    <div className={`det-toast det-toast--${toast.tipo}`}>
      <span>{toast.tipo === "error" ? "⚠️" : "ℹ️"}</span>
      <p>{toast.mensaje}</p>
    </div>
  );
            ...
            <section className="det-section det-section--plato">
              ...
              <div className="det-plato">
                ...
                <div className="det-plato__info">
                  ...
                  {restaurante.plato.descripcion && <p>{restaurante.plato.descripcion}</p>}
                </div>
              </div>
            </section>
          )}
        ...
        <aside className="det-aside">
            ...
            {votado ? (
              <div className="det-vote-box__confirmado">
                ...
                <button
                  ...
                  style={{ marginTop: "12px", backgroundColor: "#e74c3c" }}
              ...
              <>
                <p className="det-vote-box__label">
                  {user ? "¿Es tu favorito? ¡Dale tu voto!" : "Inicia sesión para votar por este restaurante"}
                </p>
          ...
          <div className="det-info-box">
            ...
            {restaurante.direccion && (
              <div className="det-info-item"><span>📍</span><span>{restaurante.direccion}</span></div>
            )}
            {restaurante.contacto && (
              <div className="det-info-item"><span>📞</span><span>{restaurante.contacto}</span></div>
            )}
          </div>
